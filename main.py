                    ...
                    if (tasks.length > 0) {{
                        const gantt = new Gantt("#gantt-container", tasks, {{
                            ...
                            on_date_change: function(task, start, end) {{
                                // Update task dates when bar is dragged or resized
                            ...
                            on_progress_change: function(task, progress) {{
                                // Update task progress when bar progress is changed
                                fetch('/tasks/' + task.id + '/update-progress', {{
                                    method: 'POST',
                                    headers: {{'Content-Type': 'application/json'}},
                                    body: JSON.stringify({{
                                        progress: progress
            ...
            Div(
                ...
                Script("""
                    function playDingSound() {
                        ...
                        oscillator.frequency.value = 800; // High-pitched ding
                    ...
                    function toggleEditForm(taskId) {
                        ...
                        const errorMsg = document.getElementById('error-' + taskId);
                    ...
                    function validateDates(taskId) {
                        const startDate = document.getElementById('start-' + taskId).value;
                        const endDate = document.getElementById('end-' + taskId).value;
                        const errorMsg = document.getElementById('error-' + taskId);
                        
                        if (startDate && endDate && new Date(endDate) < new Date(startDate)) {
                            errorMsg.textContent = '⚠️ End date cannot be before start date!';
                    ...
                    *[Div(
                        Div(
                            ...
                            Div(
                                Form(
                                    Div(
                                        Div(
                                            ...
                                            Input(
                                                ...
                                                id=f"start-{task.id}",
                                                onchange=f"validateDates({task.id})"
                                            ),
                                            cls="form-group",
                                        ...
                                        Div(
                                            ...
                                            Input(
                                                type="date", 
                                                name="end_date", 
                                                value=task.end_date.strftime('%Y-%m-%d'),
                                                id=f"end-{task.id}",
                                                onchange=f"validateDates({task.id})"
                                            ),
                                            cls="form-group",
                                    ...
                                    Div(id=f"error-{task.id}", cls="error-message"),
                                    Button("Save Changes", type="submit", style="margin-top: 10px;", onclick=f"return validateDates({task.id})"),
                                    method="post",
                                    action=f"/tasks/{task.id}/update?project_id={project_id}"
                                ),
                                cls="task-edit-form",
                                id=f"edit-form-{task.id}"
                            ),
                        ...
                        Div(
                            Button(
                                ...
                                cls="edit-btn", 
                                type="button",
                                onclick=f"toggleEditForm({task.id})"
                            ),
                            Form(
                                Button("✓ Done", cls="delete-btn", type="submit", onclick="playDingSound()"),
                                method="post",
                                action=f"/tasks/{task.id}/complete?project_id={project_id}",
                                style="display: inline;"
                            ) if not task.completed else Span("✓ Completed", style="color: #4CAF50; font-size: 12px; margin-right: 10px;"),
                            Form(
                                Button("Remove", cls="remove-btn", type="submit"),
                                method="post",
                        ...
                        cls=f"task-item {'completed' if task.completed else ''}"
...
async def post(task_id: int, request):
    """Update task dates from Gantt chart drag/drop (AJAX endpoint)."""
    ...
    try:
        ...
        if not start_date or not end_date:
            return JSONResponse({'success': False, 'error': 'Missing dates'})
...
@rt('/tasks/{task_id}/update-progress')
async def post(task_id: int, request):
    """Update task progress from Gantt chart (AJAX endpoint)."""
    from fasthtml.common import JSONResponse
    ...
    try:
        ...
        task = db.query(Task).filter(Task.id == task_id).first()
        if task:
            task.progress = min(100, max(0, progress))  # Clamp between 0-100
            db.commit()
            db.close()
            return JSONResponse({'success': True})
        else:
...
@rt('/tasks/{task_id}/update')
def post(task_id: int, start_date: str, end_date: str, project_id: int = None):
    """Update task dates from edit form."""
    db = get_db()
    task = db.query(Task).filter(Task.id == task_id).first()
    ...
    if task:
        ...
        # Validation is done client-side, but double-check here
        if end >= start:
            task.start_date = start
            task.end_date = end
            db.commit()
...
def post(task_id: int, project_id: int = None):
    ...
    if task:
        task.completed = 1
        task.progress = 100  # Auto-set to 100% when marked done
        db.commit()
        if not project_id:
            project_id = task.project_id
    db.close()
